...
class SystemUserForm extends TPage
{
    ...
    function __construct()
    {
        ...
        $login->setMask('9999999999999999999');
        $curso->setMask('AAAAAAAAAAAAAAAAAAAAA');

        $anoatul = date('Y');
        $anos = array();
        for($i = $anoatul; $i>$anoatul-80; $i--){
            $anos[$i] = $i;
        }
        $anocurso->addItems($anos);
        $anocurso->setDefaultOption( "..:: SELECIONE ::.." );
        ...
        $unit_id->addValidation("ererer", new TRequiredValidator);
        
        $this->form->addFields( [new TLabel('ID')], [$id],  [new TLabel('Name:<font color=red><b>*</b></font>')], [$name] );
        $this->form->addFields( [new TLabel('Matrícula:<font color=red><b>*</b></font>')], [$login],  [new TLabel('Email:<font color=red><b>*</b></font>')], [$email] );
        $this->form->addFields( [new TLabel('Unidade Principal:<font color=red><b>*</b></font>')], [$unit_id],  [new TLabel('Pagina Principal:<font color=red><b>*</b></font>')], [$frontpage_id] );
        $this->form->addFields( [new TLabel('Password:<font color=red><b>*</b></font>')], [$password],  [new TLabel('Confirma Password:<font color=red><b>*</b></font>')], [$repassword] );
        $this->form->addFields( [new TFormSeparator('Informações Curso')] );
        $this->form->addFields( [new TLabel('Curso:<font color=red><b>*</b></font>')], [$curso], [new TLabel('Periodo:<font color=red><b>*</b></font>')], [$periodo]);
        $this->form->addFields( [new TLabel('Ano:<font color=red><b>*</b></font>')], [$anocurso]);
        $this->form->addFields( [new TFormSeparator('Unidades:<font color=red><b>*</b></font>')] );
        $this->form->addFields( [$units] );
        $this->form->addFields( [new TFormSeparator('Grupos:<font color=red><b>*</b></font>')] );
    ...
    public static function onSave($param)
    {
        try
        {
            ...
            if( empty($object->id) )
            {
                ...
                if ( empty($object->curso) )
                {
                    throw new Exception('Por favor digite o curso');
                }
                if ( empty($object->periodo) )
                {
                    throw new Exception('Por favor digite o periodo');
                }
                if ( empty($object->anocurso) )
                {
                    throw new Exception('Por favor marque o ano');
                }
                
                
                
                $object->active = 'Y';
                $object->autorizacao = 'N';
            }
